            ...
            <div class="card m-5 border-dark">
                <div class="card-header">
                    <h5 class="mb-0 row justify-content-around">
                        ...
                        @if ($contact->user_id)
                            <button class="btn btn-link font-weight-light" type="button" style="color: #1ABCB6;">Sender is
                                a member:
                        ...
                        <form action="{{ route('admin-contact-delete') }}" method="post">
                            @csrf
                            @method('delete')
                            <input type="number" name="contactId" id="contactId" value="{{ $contact->id }}" required
                                readonly hidden>
